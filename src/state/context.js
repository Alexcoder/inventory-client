...
export const Context =({children})=>{
    ...
    const IncommingAntifoam = filteredByUser.filter(({type, category})=> type.match(incomming)  & category.match(Antifoam) )
    ...
    const OutgoingAntifoam = filteredByUser.filter(({type, category})=> type.match(outgoing)  & category.match(Antifoam) )
